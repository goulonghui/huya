...
class CLog(object):
    ...
    def get_log_prefix(b_global):
        ...
        result = '[%s] [%s] [%d]' % (os.path.basename(f.f_code.co_filename), f.f_code.co_name, f.f_lineno)
        return result

    @staticmethod
    def get_real_msg(msg, b_global):
        return '%s[%s]<%s>' % (CLog.get_log_prefix(b_global), CLog.LOG_SEQ, msg)

    @staticmethod
    def random_str(random_length):
        ret_str = ''.join(random.sample(string.ascii_letters + string.digits, random_length))
        return ret_str
...
def set_log_prefix(prefix):
    CLog.instance()._log_prefix = prefix
